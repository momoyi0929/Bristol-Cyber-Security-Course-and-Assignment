...
/**
 * =================================================================== 
 * This function Compute cryptographic hash of a file using streaming processing
  爱你的calculates the hash of a file by reading it in fixed-size
 * chunks (4KB) and incrementally updating the hash computation. This 
 * streaming approach ensures constant memory usage regardless of file size,
 * making it suitable for processing very large files efficiently.
 * 
 * The function uses OpenSSL's low-level hashing API to maintain fine-grained
 * control over the hashing process and avoid the abstraction overhead of
 * higher-level interfaces like EVP.
 * 
 * @param filename   Path to the file to be hashed (must be readable)
 * @param algorithm  Hashing algorithm to use ("sha256" or "sha512")
 * @param hash       Output buffer to store the computed hash (must be at least MAX_HASH_LEN bytes)
 ...
 * 
 * Memory Management:
 * - Uses stack-allocated buffer for file reading (no dynamic allocation per chunk)
 * - Hash output buffer is provided by caller (clear ownership semantics)
 * - All OpenSSL contexts are stack-allocated and automatically cleaned up
 * 
 * Error Handling:
 * - File opening failures (permission, not found, etc.)
 * - File reading errors (I/O failures, disk errors)
 * - OpenSSL cryptographic operation failures
 * - Algorithm specification errors
 * 
 ...
 * - Uses approved cryptographic primitives (SHA-256, SHA-512)
 * - No sensitive data retained in memory after function completion
 * - File contents are hashed as-is without interpretation
 * - All errors result in safe termination of hashing process
 * ===================================================================
 */
