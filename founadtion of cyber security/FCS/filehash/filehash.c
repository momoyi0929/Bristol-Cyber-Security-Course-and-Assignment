 ...
 * Core logic functions, Hashing functions, Utility functions
 * 
 * Features:
...
int main(int argc, char *argv[]) {
    ...
    int result = run_hash_process(filename, algorithm, expected_hash);


    // calculate time
    clock_t end_time = clock();
    double cpu_time_used = ((double) (end_time - start_time)) / CLOCKS_PER_SEC;
    fprintf(stderr, "Process completed in %.4f seconds\n", cpu_time_used);
 ...
 * It includes length, the correct number of hexadecimal digits,
 * malformed  hash specifications.
 * 
 ...
 */
int verify_hash(const unsigned char *computed_hash, unsigned int computed_len, const char *expected_hash) {
    // Convert computed binary hash to hexadecimal string for comparison
    char computed_hex[computed_len * 2 + 1];
    // Transform each byte of the binary hash to two hex digits
...
/**
 * ===================================================================
 *  Utility functions are used to provide prompts to users
 * ===================================================================
 */
void print_usage(const char *program_name) {
