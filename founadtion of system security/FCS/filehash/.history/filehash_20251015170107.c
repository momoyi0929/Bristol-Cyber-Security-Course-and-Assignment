...
int compute_file_hash(const char *filename, const char *algorithm,
    ...
    if (strcasecmp(algorithm, "sha256") == 0) {
        ...
        *hash_len = SHA512_DIGEST_LENGTH; // 64 bytes for SHA-512
        ...
        // Stream processing with same pattern as SHA-256
        while ((bytes_read = fread(buffer, 1, BUFFER_SIZE, file)) > 0) {
...
/**
 * =================================================================== 
 * This function transforms a binary hash (raw bytes) into its human-readable
...
/**
 * ===================================================================
 * Verify computed hash against expected hash value
 * 
 * This function performs the critical integrity check by comparing a computed
 * binary hash against an expected hexadecimal hash string. It converts the
 * computed binary hash to hexadecimal format and performs a case-insensitive
 * comparison to determine if the file's contents match the expected hash.
 * 
 * The verification process includes length validation to ensure the expected
 * hash has the correct number of hexadecimal digits for the given algorithm,
 * providing early detection of malformed or incorrect hash specifications.
 * 
 * @param computed_hash  Binary hash computed from the file (raw bytes)
 * @param computed_len   Length of the computed hash in bytes
 * @param expected_hash  Expected hash value as hexadecimal string (user-provided)
 * 
 * @return int          Verification result:
 *                     - 0: Hashes match (file integrity verified)
 *                     - 1: Hashes don't match or validation error
 * 
 * Verification Process:
 * 1. Validate input parameters for safety
 * 2. Check expected hash length matches computed hash length (in hex digits)
 * 3. Convert computed binary hash to hexadecimal string
 * 4. Perform case-insensitive comparison of hexadecimal strings
 * 5. Return match result with appropriate user feedback
 * 
 * Comparison Methodology:
 * - Case-insensitive comparison accommodates user input variations
 * - Hexadecimal conversion uses standard two-digit-per-byte format
 * - Length validation prevents partial or malformed hash comparisons
 * - Both hashes displayed on mismatch for debugging and verification
 * 
 * Error Conditions:
 * - NULL pointer parameters (defensive programming)
 * - Length mismatch between computed and expected hashes
 * - Hexadecimal character conversion issues
 * - Memory allocation failures in string conversion
 * 
 * Security Considerations:
 * - Timing-safe comparison could be considered for high-security applications
 * - Input validation prevents buffer overflows in string operations
 * - Clear mismatch reporting helps users identify verification failures
